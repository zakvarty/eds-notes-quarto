...
In what we have covered so far we have been very much focused on the first aspect of data science, the data. We also have consider whether our work can be reproduced and whether our results can be replicated. This shifts our focus to the second aspect, the science. 

```{r scientific-method-png}
#| echo: false
#| warning: false
#| message: false
#| fig-cap: "Cycle of scientific enquiry."
#| fig-alt: "The scientific method show as a cycle: hypotheis, literature search, design experiment, gather evidence, information synthesis, draw inference, repeat."
knitr::include_graphics(path = "images/401-production-reproducibility/scientific-method.png")
```



As data scientists, we probably like to think that we are applying the scientific method in our work.  
...
<!-- TODO: Add Bayes Sketch -->

When doing so we acknowledge that the conclusions we reach are not the truth, just our current best approximation of it. We have a usefully simplified model of the messy reality that we can share with the world. We will happily update our model as we become aware of new evidence, whether that new information supports or contradicts our current way of thinking. That is starting to sound an awful lot like Bayesian inference if you ask me.

In ideal world, this iterative approach to testing and updating the current state of knowledge is how both science and data science would progress. However, just like our models, this is a simplified and idealised description of what really happens.


## Issue: Multiple, Dependent Tests 

> Multiple testing in brief:
>
> - Projects are usually not a single hypothesis test
> 
> - Data science requires a sequence of dependent decisions, e.g. model development
> 
> - Can fool ourselves by looking at the data lots of times or ignoring sequential and dependent structures in our workflow.  


The aims of a data science project are rarely framed as a clear and unambiguous hypothesis test, for which we will design a one-time study and perform a single statistical analysis. Apart from in special cases like A/B testing we have a much more general aim for our data science projects. 

::: medium_right
![[Gelman and Loken (2013) ](http://www.stat.columbia.edu/~gelman/research/unpublished/forking.pdf) consider the issue of multiple comparisons](images/401-production-reproducibility/forking-paths-abstract.png){alt="Abstract text of Gelman and Loken (2013)."}
:::

If we want to construct a model for a given phenomenon, we're likely to try many variants of that model along the way. By taking a more relaxed approach to modelling, we run the risk of finding spurious relationships within our data that won't hold more generally. If you look for a relationship between the quantity you are trying to predict and enough samples of random noise will almost surely find a significant relationship, even though there is no true link.

<!-- TODO: Add exercise demonstrating this --->

## Issues: $p$-hacking and Publiction Bias  

::: medium_left
![Distribution of p-values in medical publications. From [Perneger and Combescure (2017)](https://doi.org/10.1016/j.jclinepi.2017.04.003)](images/401-production-reproducibility/p-values-in-medical-papers.jpg){Bar chart of p-values in medical publications, showing a large drop between 4-5 percent and 5-6 percent.}
:::
...
Studies that don't provide strong evidence against the null hypothesis rarely make it into publications, reports or the news. This is largely because of the way that scientific enquiry is rewarded in the the academy, business and the media. Funding and attention tend to go to studies which provide new insights, rather than those which aim to confirm or strengthen existing knoweldge. 
 
This systemic problem can incentivise scientists to 'massage' numbers to obtain a $p$-value less than 0.05 so that a result can be reported as "statistically significant". This process is known as $p$-hacking and can occur through deliberate malpractice. More often though, it's a result of scientists generally not receiving adequate training in statistics. As statistically trained data scientists we know that a declaration of significance is no indication of a meaningful effect size and that the conventional significance level of 5% is entirely arbitrary. However, we need to be aware that this is not the case across science and that even we aren't immune to the societal and systematic influences that favour the publication of novel results over confirmatory ones.

<!-- TODO: Expand with an example of a significant but not meaningful effect -->

These influences also lead to a much more subtle problem than direct $p$-hacking. Consider adding an obvious but unnecessary extension to a model, a example example of which might be adding a superfulous term to a regression model. Since this extension is such low hanging fruit, many scientists are likely to independently design experiments to test out whether it is worth adding to the model. Since there is truly no benefit, most of these experiments provide insufficient evidence against the null hypothesis. These studies are unlikely to get developed into published papers or technical reports, because they just support the status-quo.  However, after enough people have attempted this some scientist will get "lucky" and find a significant benefit to the model extension. Of all the experiments that were done, this is the only one that makes it onto the public record. 

All of those other studies being left in the proverbial desk drawer induces publication bias in the scientific literature. When we come to assess the state of existing knowledge, this publication bias hinders our ability to properly assess the importance of published findings, because we lack the context of all those null results that went unreported.

<!-- Add plot of true and estimated effect sizes, with unreported values greyed out -->

This same process means that the results of many more scientific studies than we would expect cannot be recreated, in is what is known as the scientific replication crisis. 
...
Reproducibility requires us to be able to recover the exact same numerical summaries that were found by the original investigator. In particular this means we should be able to reproduce the exact same point estimates and measures of uncertainty that they did, which ensures we'll draw the same conclusions as that original investigator.

When putting our work into production there are several reasons why we might require it to be reproducible. The first is logistical: production code needs to be robust and efficient - this often means your code will be re-factored, rewritten or translated into another language. If your results are not reproducible then there is no way to verify that this has been done correctly. Secondly, if a problem is identified in your work (say a customer raises a complaint that their loan application was incorrectly rejected) you need to be able to accurately recreate that instance to diagnose if there is a problem with the model and exactly what caused it. 

Note that just because findings are reproducible, that doesn't by any means imply that they're correct. We could have a very well documented but flawed analysis that is entirely reproducible and is also completely unsuitable or just plain wrong.  

In our data science projects, we have already taken several steps that greatly improve the reproducibility of our work. When we scripted, tested and documented our work we were aiming to improve the management of our project but these decisions also improve the scientific quality of our work. This puts us in a strong position relative to the scientific literature as a whole. 

We are at a point now where it is almost standard to publish data along with papers, but for a long time this was not the case. If data were available at all, this was often only by request. Nowadays, we are in a similar situation when it comes to code. It's still far from standard for the analysis code to be required and put up to detailed scrutiny as part of the peer-review process. 

With a little more context this isn't so unreasonable. Across many scientific disciplines, code-based approaches to analysis is not standard; statistical software with a graphical user interface is used instead. The idea here is to allow scientists to analyse their own data by providing statistical tools trough a combination of menus and buttons (a Graphical User Interface or GUI). However, these interfaces often leave no record of how the data were manipulated and the software itself can be highly specialised or proprietary. This combination means that even when full datasets are provided, it is often impossible for others to reproduce the original analysis.
...
This is where we need to pause and check ourselves because the same fate can easily befall us as data scientists. It take time to learn the skills and practices that allow us to work reproducibly but it also takes time to implement them in any given project. The time of an expert data practitioner doesn't come cheap, if you wait until the end of a project before you make it reproducible then you'll be too late. Time or money will have run out. 
...
Ultimately, replicability this is the core problem that statisticians get paid to worry about. If a study is repeated, will the resulting changes in the fitted model and predictions changes be small enough that the substantive conclusions of the study are not impacted? Of course, point estimates will vary slightly but do your conclusions about the existence, direction or magnitude of an effect still hold? Alternatively, if you are estimating a relationship between two variables, is the same functional form chosen as the most suitable? 

In a general scientific context, replication takes a more broad meaning and asks whether the key properties of your results could be obtained by another person running the same experiment again. In the context of getting your work put into production, we are concerned about whether your findings will still hold when applied to new instances that might differ from those we have seen already.

If you come to data science from a statistical background then you are well accustomed to these sorts of considerations. Whenever you perform a hypothesis test or compare two models, you take steps to make sure the comparison is not only valid for this particular sample, but also out-of-sample. This is the whole reason data scientists split their data into training and test sets in the first place, as an approximate test for this sort of generalisation. In each case, we are asking of ourselves: will this good performance replicate if we had different obsevations as inputs? 
...
::: medium_left
```{r monte-carlo-approximation-of-pi}
#| echo: false
#| fig-cap: "Monte Carlo approximation of $\\pi$"
#| fig-alt: "Points randomly located on [-1,1] x [-1,1], with the unit circle overlaid."
library(ggplot2)
set.seed(1234)
...
- runs of the optimisation procedure?
- starting points?
...
<!-- TODO: Replace with actual gradient descent example -->

If optimisation routines are used in parameter estimation, we have to ensure that the results they find for a particular data set are reproducible for a given configuration. Depending on the function we are trying to optimise, this might involve assessing stablility of the optimisation algortihm to the particular values in a data set, the initial set of starting parameters and step size used, or the choice of learning rate (which controls how that step size changes) and maximum number of iterations to perform. 
...
If the optimisation fails to converge, can your reproduce that failure in order to diagnose and fix the problem? This can be particularly tricky when the optimisation routine itself uses Monte Carlo methods, such as stochastic gradient descent or simulated annealing. 

### (Pseudo-)Random Numbers

Sometimes we have stochastic elements to our work that we can't use brute force to eliminate. Perhaps doing so is beyond our computational budget or technical abilities or perhaps the number of realisations is an important aspect of our study that has to be held fixed for us to draw valid conclusions. 

Fortunately, in computing it's not common to use truly random numbers. Instead, what we usually have is a complicated but deterministic function that generates a sequence of numbers that are statistically indistinguishable from a series of independent random variates. These values, which are calculated using a deterministic algorithm but appear to be random, are known as pseudo-random numbers. 

Within a pseudo-random number generator, the next value in the sequence of pseudo-random numbers is generated based on value the current one. This means that by setting the starting point we can produce the same sequence of pseudo-random variates each time we run the code. In R we set this starting point using `set.seed()`.
...
When writing code that's executed in parallel across multiple cores or processors, you have to carefully consider whether you want to give each parallel thread the same seed value. The correct decision here is context specific and depends on the interpretation of the random variates you will be generating. If you are making a comparison between threads then it might be important that the random aspects are kept as similar as possible across threads so that a fair comparison can be made. However, if you are using a parallel implementation only for the associated speed gains then this might not be important at all.

Finally, it's important to be wary of the quality of pseudo-random number generation, particularly when interfacing R with other programming languages. R was developed as a _statistical_ programming language and most other languages are not as focused on statistical rigour. Different languages may use different algorithms for generating pseudo-random numbers, and the quality of the generated numbers can vary greatly. Additionally, it's important to make sure that when values are passed between languages that seeds are appropriately passed between the languages too, to ensure that the intended sequence of random numbers is generated. 
...
While randomness is a key part of most data science workflows, it can lead to reproducibility nightmares. We can manage these by appealing to the stability of averages in large samples and by explicitly setting the sequence of pseudo-random numbers that we generate using `set.seed()`. 
