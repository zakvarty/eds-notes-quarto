...
```{r,echo = FALSE, out.width="100%", fig.cap="Coffee consumption, visualised. Jaime Serra Palou.", fig.alt="12 sheets of paper arranged to make a calendar. Each sheet has been folded into squares, with each square and used as a coaster for a single day. Ring-shaped coffee stains visualise daily coffee consumption over the year, which is greatest in May and least in December."}
...
```{r,echo = FALSE, out.width="100%", fig.cap="Caffeination vs sleep, shown in lego. Elsie Lee-Robbins", fig.alt="A 2D barplot with axes for how caffeinated staf members are and how much sleep they have had. The height of stacked bricks appears to show a fairly uniform distribution, other than there being lots of people who have not had much sleep or caffeine."}
knitr::include_graphics(
  path = "images/303-data-visualisation/lego-coffee.jpg")
```
...
```{r,echo = FALSE, out.width="100%", fig.cap="Examples of preattentive attributes", fig.alt="An illustration of how several pre-attentive attributes can be used to draw the attention of the viewer. In each case there is a grid of nine hollow squares and attention is drawn to a subset of those but altering the following attributes: shape, enclosure, line weight, fill colour/saturation, size and position. Similar examples are given of line length and line curvature."}
knitr::include_graphics(
...
```{r,echo = FALSE, out.width="100%", fig.cap="Some default colour scales in R", fig.alt="Six default continuous colour scales used by various R packages, including gradient and diverging and rainbow palette types."}
knitr::include_graphics(
...
```{r,echo = FALSE, out.width="100%", fig.cap="Desatureated colour scales in R", fig.alt="Six default continuous colour scales used by various R packages, this time shown in greyscale."}
knitr::include_graphics(
